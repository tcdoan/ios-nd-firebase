    ...
    func fetchConfig() {
        ...
        remoteConfig.fetch(withExpirationDuration: expirationDuration) { (status, error ) in
            if status == .success {
                ...
                let friendlyMsgLen = self.remoteConfig["friendly_msg_length"]
                if friendlyMsgLen.source != .static {
                    self.msglength = friendlyMsgLen.numberValue
                    print("Msg Length: \(self.msglength)")
                }
            } else {
...
extension FCViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let imageURL = message[Constants.MessageFields.imageUrl] {
            ...
            Storage.storage().reference(forURL: imageURL).getData(maxSize: Int64.max) { (data, error) in
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // Unpack data from firebase data snapshot
